...
/* CZet extension: compile-time evaluation of calls to CZet
   constexpr functions.  If every argument is a constant expression, the
   function body is executed (supporting local variables, arithmetic and
   loop/pointer-free control flow lowered to gotos, and single/multiple
   returns) and the folded return value is produced.  Returns the fully
   folded constant expression, or NULL_TREE if the call cannot be
   evaluated this way.  */

/* Result codes for c_fold_cx_exec.  */
enum c_fold_cx_status
{
  CX_FAILED = 0,	/* not evaluable as a compile-time constant */
  CX_FINISHED,		/* statements ran to the end, no return */
  CX_RETURNED		/* a return statement was reached */
};

/* Environment mapping parameters and local variables to their current
   constant values.  */
typedef hash_map<tree, tree> c_fold_cx_env;

/* Walk callback replacing PARM_DECL / local VAR_DECL parameters and
   variables (present in the environment) with their constant values.  */
static tree
c_fold_cx_subst (tree *tp, int *walk_subtrees, void *data_)
{
  c_fold_cx_env *env = (c_fold_cx_env *) data_;
  if ((VAR_P (*tp) || TREE_CODE (*tp) == PARM_DECL))
    if (tree *val = env->get (*tp))
      {
	*tp = *val;
	*walk_subtrees = 0;
      }
...
/* Evaluate EXPR to a compile-time constant under ENV.  Substitutes the
   current env values and fully folds.  Returns the constant, or
   NULL_TREE if EXPR is not evaluable as a constant.  */
static tree
c_fold_cx_eval (tree expr, c_fold_cx_env &env, bool in_init)
{
  if (expr == NULL_TREE)
    return NULL_TREE;
  expr = unshare_expr (expr);
  walk_tree (&expr, c_fold_cx_subst, &env, NULL);
  bool c1 = true, c2 = true;
  expr = c_fully_fold_internal (expr, in_init, &c1, &c2, false, false);
  if (TREE_CONSTANT (expr))
    return expr;
  return NULL_TREE;
}

/* Execute the statements STMS under ENV, controlling flow through
   labels and gotos.  Returns CX_*; on CX_RETURNED, *RET holds the
   folded return value.  */
static c_fold_cx_status
c_fold_cx_exec (tree stms, c_fold_cx_env &env, bool in_init, tree &ret)
{
  ret = NULL_TREE;
  auto_vec<tree> list;
  if (stms && TREE_CODE (stms) == STATEMENT_LIST)
    for (tree_stmt_iterator it = tsi_start (stms);
	 !tsi_end_p (it); tsi_next (&it))
      list.safe_push (tsi_stmt (it));
  else if (stms)
    list.safe_push (stms);

  /* Map labels to their statement index.  */
  hash_map<tree, unsigned> labels;
  unsigned n = list.length ();
  for (unsigned i = 0; i < n; i++)
    if (list[i] && TREE_CODE (list[i]) == LABEL_EXPR)
      labels.put (LABEL_EXPR_LABEL (list[i]), i);
  ...
  while (pc < n)
	...
	case GOTO_EXPR:
	  {
	    unsigned *to = labels.get (GOTO_DESTINATION (st));
	    if (to == NULL)
	      return CX_FAILED;
	    pc = *to;
	    continue;
	...
	case COND_EXPR:
	  {
	    tree cond = c_fold_cx_eval (TREE_OPERAND (st, 0), env, in_init);
	    if (cond == NULL_TREE)
	      return CX_FAILED;
	    tree taken = integer_zerop (cond) ? TREE_OPERAND (st, 2)
					     : TREE_OPERAND (st, 1);
	    if (taken == NULL_TREE || TREE_CODE (taken) != GOTO_EXPR)
	      return CX_FAILED;
	    unsigned *to = labels.get (GOTO_DESTINATION (taken));
	    if (to == NULL)
	      return CX_FAILED;
	    pc = *to;
	    continue;
	...
	case DECL_EXPR:
	  {
	    tree d = DECL_EXPR_DECL (st);
	    if (d && VAR_P (d) && DECL_INITIAL (d) != NULL_TREE)
	      {
		tree v = c_fold_cx_eval (DECL_INITIAL (d), env, in_init);
		if (v == NULL_TREE)
		  return CX_FAILED;
		env.put (d, v);
	      }
	...
	case MODIFY_EXPR:
	  {
	    tree lhs = TREE_OPERAND (st, 0);
	    tree rhs = c_fold_cx_eval (TREE_OPERAND (st, 1), env, in_init);
	    if (rhs == NULL_TREE)
	      return CX_FAILED;
	    if (TREE_CODE (lhs) == RESULT_DECL)
	      {
		/* Assignment to the result; final value comes from the
		   RETURN_EXPR, so nothing further to record here.  */
		continue;
	      }
	...
	case PREDECREMENT_EXPR:
	  {
	    tree lhs = TREE_OPERAND (st, 0);
	    tree *val = env.get (lhs);
	    if (val == NULL)
	      return CX_FAILED;
	    bool inc = (TREE_CODE (st) == POSTINCREMENT_EXPR
			|| TREE_CODE (st) == PREINCREMENT_EXPR);
	    tree delta = build_int_cst (TREE_TYPE (lhs), inc ? 1 : -1);
	    tree sum = fold_build2 (PLUS_EXPR, TREE_TYPE (lhs), *val, delta);
	    tree nv = c_fold_cx_eval (sum, env, in_init);
	    if (nv == NULL_TREE)
	      return CX_FAILED;
	    env.put (lhs, nv);
	    continue;
	...
	case BIND_EXPR:
	  {
	    /* Enter a nested scope: declare its non-initialized-free
	       variables with their initializers, then run its body.  */
	    for (tree v = BIND_EXPR_VARS (st); v; v = DECL_CHAIN (v))
	      if (VAR_P (v) && DECL_INITIAL (v) != NULL_TREE)
		{
		  tree val = c_fold_cx_eval (DECL_INITIAL (v), env, in_init);
		  if (val == NULL_TREE)
		    return CX_FAILED;
		  env.put (v, val);
		}
	    tree vr = NULL_TREE;
	    c_fold_cx_status s = c_fold_cx_exec (BIND_EXPR_BODY (st), env,
						 in_init, vr);
	    if (s == CX_FAILED)
	      return CX_FAILED;
	    if (s == CX_RETURNED)
	...
	case STATEMENT_LIST:
	  {
	    tree vr = NULL_TREE;
	    c_fold_cx_status s = c_fold_cx_exec (st, env, in_init, vr);
	    if (s == CX_FAILED)
	      return CX_FAILED;
	    if (s == CX_RETURNED)
	      {
	...
	case RETURN_EXPR:
	  {
	    tree val = TREE_OPERAND (st, 0);
	    if (val == NULL_TREE)
	      return CX_FAILED;
	    if (TREE_CODE (val) == MODIFY_EXPR
		&& TREE_CODE (TREE_OPERAND (val, 0)) == RESULT_DECL)
	      val = TREE_OPERAND (val, 1);
	    ret = c_fold_cx_eval (val, env, in_init);
	    return ret != NULL_TREE ? CX_RETURNED : CX_FAILED;
	  }

	default:
	  /* Anything we cannot interpret is not a compile-time
	     constant.  */
	  return CX_FAILED;
	}
    }
  /* Ran to the end of this scope without hitting a return.  */
  return CX_FINISHED;
}

/* If EXPR is a call to a CZet constexpr function with all-constant
   arguments, evaluate the function body and fold the result.  Returns
   the folded result or NULL_TREE if not applicable.  */
static tree
c_fold_constexpr_call (tree expr, bool in_init)
{
  ...
  if (fndecl == NULL_TREE || TREE_CODE (fndecl) != FUNCTION_DECL
      || !C_DECL_DECLARED_CONSTEXPR_FN (fndecl))
    return NULL_TREE;

  /* The function must already be defined so its body is available.  */
  tree decl_tree = DECL_SAVED_TREE (fndecl);
  if (decl_tree == NULL_TREE)
    return NULL_TREE;

  /* Count the parameters; the call must supply exactly one argument
     per parameter (no defaults or varargs in C).  */
  unsigned nparams = 0;
  for (tree p = DECL_ARGUMENTS (fndecl); p; p = DECL_CHAIN (p))
    ++nparams;
  if (call_expr_nargs (expr) != nparams)
    return NULL_TREE;

  /* Fold each argument to a constant and bind it to its parameter in
     the initial environment.  */
  c_fold_cx_env env;
  tree parm = DECL_ARGUMENTS (fndecl);
  for (unsigned i = 0; i < nparams; i++, parm = DECL_CHAIN (parm))
    {
      bool c1 = true, c2 = true;
      tree arg = c_fully_fold_internal (CALL_EXPR_ARG (expr, i), in_init,
					&c1, &c2, false, false);
  ...
  tree ret = NULL_TREE;
  c_fold_cx_status s = c_fold_cx_exec (decl_tree, env, in_init, ret);
  if (s != CX_RETURNED)
    return NULL_TREE;
  return ret;
}
		       ...
		       bool *maybe_const_itself, bool for_int_const, bool lval)
{
  ...
  if (kind == tcc_vl_exp)
    {
      /* CZet extension: a call to a CZet constexpr function whose
	 arguments are all constant is itself a constant expression and
	 is evaluated here.  */
      if (code == CALL_EXPR && flag_isoczet)
	{
	  tree folded = c_fold_constexpr_call (expr, in_init);
	  if (folded != NULL_TREE)
	    {
	      *maybe_const_operands = true;
	      ret = folded;
	      goto out;
	    }
	}

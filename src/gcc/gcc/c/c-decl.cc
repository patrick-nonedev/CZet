...
/* ===================== CZet generic functions =====================
   A generic function "void f<T,U>(...)" is kept as a template whose
   prototype carries CZET_TPL_TYPE placeholders.  It is never emitted;
   each call "f<A,B>(...)" monomorphizes the body and emits a concrete
   instance.  The machinery below manages placeholder construction, the
   current type-parameter scope while parsing a definition, storage of the
   deferred body, and tree substitution at instantiation.  */

/* Type-parameter list (chain of IDENTIFIER_NODEs) for each generic function
   definition, keyed by its FUNCTION_DECL.  Kept as a GGC-rooted linked chain
   (the static hash_map is not markable by gengtype and the collector runs
   between definitions that store and retrieve these entries).  */
struct GTY(()) czet_generic_parms_ent
{
  tree fn;
  tree params;
  struct czet_generic_parms_ent *next;
};

static GTY(()) czet_generic_parms_ent *czet_generic_parms_list;

/* The generic function currently being defined (or NULL).  Its type
   parameters are active while its prototype AND body are parsed.  */
static tree czet_current_generic_fn;

/* The currently active type-parameter name list (chain of IDENTIFIER_NODEs),
   active while a generic definition's prototype/body is parsed.  It may be
   set during declarator parsing (before the definition decl exists).  */
static tree czet_current_generic_parms;

tree
czet_generic_params (tree fn)
{
  if (!fn || czet_generic_parms_list == NULL)
    return NULL_TREE;
  for (czet_generic_parms_ent *e = czet_generic_parms_list; e;
       e = e->next)
    if (e->fn == fn)
      return e->params;
  return NULL_TREE;
...
void
czet_set_generic_params (tree fn, tree params)
{
  for (czet_generic_parms_ent *e = czet_generic_parms_list; e;
       e = e->next)
    if (e->fn == fn)
  ...
  czet_generic_parms_ent *e = ggc_alloc<czet_generic_parms_ent> ();
  e->fn = fn;
  e->params = params;
  e->next = czet_generic_parms_list;
  czet_generic_parms_list = e;
}

/* Instantiation bindings: while re-parsing a generic definition for a
   concrete instantiation, this maps each type-parameter NAME to the concrete
   TYPE bound at the call site.  NULL when no instantiation is active.  */
static hash_map<tree, tree> *czet_bindings_map;
static tree czet_binding_template;

void
czet_set_instantiation_bindings (tree templ, tree types)
{
  if (czet_bindings_map == NULL)
    czet_bindings_map = new hash_map<tree, tree>;
  czet_bindings_map->empty ();
  czet_binding_template = templ;
  tree param_names = czet_generic_params (templ);
  for (tree p = param_names, t = types; p && t;
       p = TREE_CHAIN (p), t = TREE_CHAIN (t))
    czet_bindings_map->put (TREE_VALUE (p), TREE_VALUE (t));
}

/* End an active instantiation: drop all concrete type bindings and clear the
   binding-template marker.  */
void
czet_clear_instantiation_bindings (void)
{
  czet_binding_template = NULL_TREE;
  if (czet_bindings_map)
    czet_bindings_map->empty ();
}

/* The most recently created monomorphized instance decl (during replay).  */
tree czet_last_instance;
tree czet_replay_decl;

/* Build a unique identifier for the instantiation of TEMPL with the currently
   bound types, e.g. "f__czet3".  A running counter keeps distinct
   <template,types> combos distinct; the caller short-circuits on duplicates.  */
tree
czet_instance_name (tree templ ATTRIBUTE_UNUSED)
{
  static unsigned long counter;
  char tmp[64];
  snprintf (tmp, sizeof tmp, "f__czet%lu", counter++);
  return get_identifier (tmp);
}

/* walk_tree callback: replace each CZET_TPL_TYPE placeholder whose TYPE_NAME
   is bound in the (decl_tree_map) DATA with its concrete type.  */
static tree czet_subst_walk (tree *, int *, void *);
tree czet_instantiate_type (struct c_parser *, tree, tree);
static tree
czet_subst_walk (tree *tp, int *walk_subtrees, void *data)
{
  hash_map<tree, tree> *bound = (hash_map<tree, tree> *) data;
  if ((TREE_CODE (*tp) == RECORD_TYPE || TREE_CODE (*tp) == UNION_TYPE)
      && TYPE_NAME (*tp)
      && TREE_CODE (TYPE_NAME (*tp)) == IDENTIFIER_NODE)
    {
      tree tag = TYPE_NAME (*tp);
      if (czet_get_type_template_type (tag) != NULL_TREE)
	{
	  /* Generic-struct reference nested inside another type (e.g. a
	     pointer or array).  Rebuild its concrete argument list from the
	     template's parameter names and the active binding map, then
	     produce the canonical instantiation.  */
	  tree tparams = czet_get_type_template_params (tag);
	  tree tl = NULL_TREE, tail = NULL_TREE;
	  for (tree p = tparams; p; p = TREE_CHAIN (p))
	    {
	      if (p == error_mark_node || TREE_VALUE (p) == NULL_TREE)
		continue;
	      tree *slot = bound->get (TREE_VALUE (p));
	      tree conc = slot ? *slot : integer_type_node;
	      tree node = tree_cons (NULL_TREE, conc, NULL_TREE);
	      if (tail)
		TREE_CHAIN (tail) = node;
	      else
		tl = node;
	      tail = node;
	    }
	  tree inst = czet_instantiate_type (NULL, tag, tl);
	  if (inst)
	    {
	      *tp = inst;
	      *walk_subtrees = 0;
  ...
  if (TREE_CODE (*tp) == CZET_TPL_TYPE)
    {
      tree t = *tp;
      tree name = TYPE_NAME (t);
      if (name)
        {
          tree *slot = bound->get (name);
          if (slot)
            {
              *tp = *slot;
              return NULL_TREE;
      ...
      *tp = integer_type_node; /* fallback */
      return NULL_TREE;
    }
  if (TREE_CODE (*tp) == INTEGER_TYPE && TYPE_NAME (*tp)
      && TREE_CODE (TYPE_NAME (*tp)) == IDENTIFIER_NODE)
    {
...
/* Build the concrete prototype type for an instantiation of TEMPL with the
   given concrete types, by substituting the placeholders in TEMPL's return
   and parameter types.  TYPE_LIST is a chain (TREE_LIST) of concrete types
   bound to TEMPL's template parameters in order.  */
static tree czet_subst_type (tree t, hash_map<tree, tree> *bound);
tree
czet_materialize (tree templ, tree type_list)
{
  hash_map<tree, tree> *bound = new hash_map<tree, tree>;
  tree params = czet_generic_params (templ);
  for (tree p = params, ty = type_list; p && ty;
       p = TREE_CHAIN (p), ty = TREE_CHAIN (ty))
    {
      tree name = TREE_VALUE (p);
      tree conc = TREE_VALUE (ty);
      bound->put (name, conc);
    }
  tree proto = TREE_TYPE (templ); /* FUNCTION_TYPE */
  /* Build arg type list with substitution.  */
  vec<tree> argvec = vNULL;
  for (tree a = TYPE_ARG_TYPES (proto); a && a != void_list_node;
       a = TREE_CHAIN (a))
    {
      tree t = TREE_VALUE (a);
      t = czet_subst_type (t, bound);
      argvec.safe_push (t);
    }
  tree ret = TREE_TYPE (proto);
  ret = czet_subst_type (ret, bound);
  tree tl = NULL_TREE;
  for (unsigned i = argvec.length (); i > 0; i--)
    tl = tree_cons (NULL_TREE, argvec[i - 1], tl);
  tl = chainon (tl, void_list_node);
  tree ftype = build_function_type (ret, tl);
  delete bound;
  return ftype;
}

/* Substitute concrete types for placeholders in TYPE, but if TYPE is a
   generic struct/union template reference, produce the canonical concrete
   record via the type-instance cache so it unifies with plain use sites.  */
static tree
czet_subst_type (tree t, hash_map<tree, tree> *bound)
{
  if (t == NULL_TREE || t == error_mark_node)
    return t;
  /* Rebuild pointer-to-* and array wrappers around a safely-substituted
     pointed element so the shared template's type nodes are never mutated
     in place (which would otherwise corrupt other instantiations).  */
if (POINTER_TYPE_P (t))
    {
      tree pt = TREE_TYPE (t);
      tree npt = czet_subst_type (pt, bound);
      if (npt == pt)
	return t;
      /* Rebuild the pointer by copying the shared template's pointer node
	 and pointing the copy at the substituted (per-instance) record.  We
	 deliberately bypass build_pointer_type: it canonicalizes the pointed-to
	 against the template, collapsing the instance identity.  Copying the
	 wrapper preserves a normal pointer while letting `p->f` resolve fields
	 against the concrete record.  */
      tree mptr = copy_node (t);
      TREE_TYPE (mptr) = npt;
      if (TREE_CODE (mptr) == POINTER_TYPE)
	{
	  TYPE_POINTER_TO (npt) = NULL_TREE;
	  TYPE_MAIN_VARIANT (mptr) = mptr;
	  TYPE_NEXT_VARIANT (mptr) = NULL_TREE;
	  TYPE_CANONICAL (mptr) = mptr;
	}
      return mptr;
    }
  if (TREE_CODE (t) == ARRAY_TYPE)
    {
      tree et = TREE_TYPE (t);
      tree net = czet_subst_type (et, bound);
      if (net == et)
	return t;
      return build_array_type (net, TYPE_DOMAIN (t));
    }
  if (t && (TREE_CODE (t) == RECORD_TYPE || TREE_CODE (t) == UNION_TYPE)
      && TYPE_NAME (t)
      && TREE_CODE (TYPE_NAME (t)) == IDENTIFIER_NODE)
    {
      tree tag = TYPE_NAME (t);
      if (czet_get_type_template_type (tag) != NULL_TREE)
	{
	  /* Recover the concrete type-argument list from the template's own
	     type parameters instead of its fields: fields may be nested
	     (e.g. a tagged union whose first member is an anonymous union), so
	     only the parameter bindings carry the true <T,U,...> arguments.  */
	  tree tl = NULL_TREE, tail = NULL_TREE;
	  for (tree p = czet_get_type_template_params (tag);
	       p; p = TREE_CHAIN (p))
	    {
	      if (p == error_mark_node)
		continue;
	      tree pname = TREE_VALUE (p);
	      tree *slot = bound->get (pname);
	      tree ctype = slot ? *slot : NULL_TREE;
	      if (ctype == NULL_TREE || ctype == error_mark_node)
		return t;	/* unresolved; leave shared template alone */
	      tree node = tree_cons (NULL_TREE, ctype, NULL_TREE);
	      if (tail)
		TREE_CHAIN (tail) = node;
	      else
		tl = node;
	  ...
	  if (tl == NULL_TREE)
	    return t;
	  return czet_instantiate_type (NULL, tag, tl);
	}
      /* Not a registered template: already a concrete instance (or plain
	 record).  No in-place mutation needed.  */
      return t;
    }
  /* Generic type-parameter placeholder bound to a concrete type.  */
  if ((TREE_CODE (t) == CZET_TPL_TYPE
       || (TREE_CODE (t) == INTEGER_TYPE && TYPE_NAME (t)
	   && TREE_CODE (TYPE_NAME (t)) == IDENTIFIER_NODE))
      && TYPE_NAME (t))
    {
      tree *slot = bound->get (TYPE_NAME (t));
      if (slot)
	return *slot;
    }
  /* Ordinary leaf type with nothing to substitute.  */
  return t;
}

/* Registry mapping (template, concrete-type-summary) -> instance decl, so a
   given <template,types> is only instantiated once.  */
static hash_map<tree, tree> *czet_instance_registry;

/* Build and return a forward declaring instance FUNCTION_DECL for TEMPL and
   the given concrete TYPE_LIST, with a unique mangled name.  */
tree
czet_build_instance_decl (tree templ, tree type_list)
{
  /* Compute a stable dedup key from the concrete types.  */
  char key[512];
  size_t off = snprintf (key, sizeof key, "%p:", (void *) templ);
  for (tree ty = type_list; ty && off < sizeof key - 1;
       ty = TREE_CHAIN (ty))
    {
      tree t = TREE_VALUE (ty);
      const char *nm = NULL;
      if (t && TYPE_NAME (t) != NULL_TREE
          && (TREE_CODE (t) == INTEGER_TYPE
              || TREE_CODE (t) == BITINT_TYPE)
          && TREE_CODE (TYPE_NAME (t)) == TYPE_DECL)
        nm = IDENTIFIER_POINTER (DECL_NAME (TYPE_NAME (t)));
      HOST_WIDE_INT bits = -1;
      if (t && (TREE_CODE (t) == INTEGER_TYPE
                || TREE_CODE (t) == BITINT_TYPE))
        bits = tree_to_shwi (TYPE_SIZE (t));
      off += snprintf (key + off, sizeof key - off, "|%s@%ld",
                       nm ? nm : "?", (long) bits);
    }
  tree keyid = get_identifier (key);
  if (czet_instance_registry == NULL)
    czet_instance_registry = new hash_map<tree, tree>;
  tree *ex = czet_instance_registry->get (keyid);
  if (ex && *ex != NULL_TREE)
    return *ex;

  tree ftype = czet_materialize (templ, type_list);
  tree id = czet_instance_name (templ);
  tree decl = build_decl (DECL_SOURCE_LOCATION (templ), FUNCTION_DECL, id, ftype);
  TREE_PUBLIC (decl) = 1;
  DECL_EXTERNAL (decl) = 0;
  DECL_SOURCE_LOCATION (decl) = DECL_SOURCE_LOCATION (templ);
  DECL_RESULT (decl) = NULL_TREE;
  czet_instance_registry->put (keyid, decl);
  return decl;
}

/* Remember which generic template is being re-parsed (used by start_function
   while instantiating).  */
void
czet_set_binding_template (tree templ)
{
  czet_binding_template = templ;
}

/* True while replaying a concrete instantiation (type bindings active).  */
bool
czet_instantiation_active (void)
{
  return czet_binding_template != NULL_TREE;
}

/* True when FN is a generic (template) function.  */
bool
czet_is_generic (tree fn)
{
  return fn != NULL && C_DECL_GENERIC_TEMPLATE (fn);
}

/* The active type-parameter scope (list of IDENTIFIER_NODEs) while a
   generic function definition is being parsed.  */
tree
czet_current_generic_params_scope (void)
{
  return czet_current_generic_parms;
}

/* Set/clear the current generic definition scope.  */
void
czet_push_generic_fn (tree fn)
{
  czet_current_generic_fn = fn;
}
void
czet_pop_generic_fn (void)
{
  czet_current_generic_fn = NULL_TREE;
  czet_current_generic_parms = NULL_TREE;
}

/* Activate a type-parameter scope (during declarator parsing, before
   start_function has created the definition decl).  The list is a chain of
   c_typespec/identifier values (IDENTIFIER_NODEs).  */
void
czet_set_generic_scope (tree params)
{
  czet_current_generic_parms = params;
}

/* Build and return the placeholder type for type parameter NAME.  The
   placeholder is a distinct INTEGER_TYPE (int-like) so it can be used directly
   as a complete type and in expressions; substitution at instantiation time
   identifies it by its IDENTIFIER TYPE_NAME.  */
tree
czet_make_type_parm (tree name)
{
  tree t = build_distinct_type_copy (integer_type_node);
  TYPE_NAME (t) = name;
  TYPE_CONTEXT (t) = czet_current_generic_fn;
  TYPE_CANONICAL (t) = t;
  return t;
}

/* If NAME is one of the currently active generic type parameters, return its
   placeholder type; otherwise NULL.  During an instantiation, the concrete
   bound TYPE is returned instead.  */
tree
czet_param_placeholder (tree name)
{
  if (czet_bindings_map)
    {
      tree *slot = czet_bindings_map->get (name);
      if (slot)
	return *slot;
    }
  tree scope = czet_current_generic_parms;
  for (tree p = scope; p; p = TREE_CHAIN (p))
    if (p != error_mark_node && TREE_VALUE (p) == name)
      return czet_make_type_parm (name);
  return NULL_TREE;
}

/* ------------------------------------------------------------------ *
 * CZet type (struct/union) templates.
 *
 * A generic struct/union definition `struct box<T,U> { ... };` is parsed
 * under an active generic scope, so its field types become placeholder
 * types.  The resulting record is registered here as a *type template*
 * (tag -> params, template record).  At a use site `struct box<int,U> x`
 * the template is monomorphized at the tree level by copying the template
 * record and substituting each field's placeholder type with the concrete
 * argument, then laying the copy out as a distinct record type.
 * ------------------------------------------------------------------ */

static hash_map<tree, tree> *czet_type_template_type_map;
static hash_map<tree, tree> *czet_type_template_params_map;
/* Per-tag list of already-instantiated concrete types (TREE_LIST of records),
   keyed by tag in a map, to dedup repeated `tag<T,U>` uses.  */
static hash_map<tree, tree> *czet_type_inst_cache;

/* Register a generic struct/union definition.  TAG is its identifier,
   PARAMS the chain of type-parameter names, TEMPLATE_TYPE the parsed record
   (fields are placeholder-typed).  */
void
czet_set_type_template (tree tag, tree params, tree template_type)
{
  if (czet_type_template_type_map == NULL)
    czet_type_template_type_map = new hash_map<tree, tree>;
  if (czet_type_template_params_map == NULL)
    czet_type_template_params_map = new hash_map<tree, tree>;
  czet_type_template_type_map->put (tag, template_type);
  czet_type_template_params_map->put (tag, params);
}

/* The placeholder record type of generic struct/union TAG, or NULL.  */
tree
czet_get_type_template_type (tree tag)
{
  if (czet_type_template_type_map == NULL)
    return NULL_TREE;
  tree *slot = czet_type_template_type_map->get (tag);
  return slot ? *slot : NULL_TREE;
}

/* The chain of type-parameter names of generic struct/union TAG.  */
tree
czet_get_type_template_params (tree tag)
{
  if (czet_type_template_params_map == NULL)
    return NULL_TREE;
  tree *slot = czet_type_template_params_map->get (tag);
  return slot ? *slot : NULL_TREE;
}

/* Substitute concrete types for placeholders in a *whole* aggregate type T,
   recursing into nested record/union member types (which walk_tree does not
   descend into).  Returns a freshly-copied, re-laid-out type; nested
   aggregates get the same treatment so their members are concrete too.  */
static tree czet_deep_subst (tree t, hash_map<tree, tree> *bound);

/* Deep-copy a FIELD_DECL and its type, substituting placeholders per BOUND.  */
static tree
czet_copy_field (tree f, hash_map<tree, tree> *bound)
{
  tree nf = copy_node (f);
  tree ft = TREE_TYPE (f);
  ft = czet_deep_subst (ft, bound);
  TREE_TYPE (nf) = ft;
  /* Clear layout state so finish_struct recomputes it for the new type.  */
  DECL_SIZE (nf) = NULL_TREE;
  DECL_SIZE_UNIT (nf) = NULL_TREE;
  DECL_FIELD_OFFSET (nf) = NULL_TREE;
  DECL_FIELD_BIT_OFFSET (nf) = NULL_TREE;
  DECL_MODE (nf) = VOIDmode;
  SET_DECL_ALIGN (nf, 0);
  return nf;
}

/* Recurse through aggregate member types, substituting placeholders that
   walk_tree would miss inside nested record/union fields.  */
static tree
czet_deep_subst (tree t, hash_map<tree, tree> *bound)
{
  if (t == NULL_TREE || t == error_mark_node)
    return t;
  if (TREE_CODE (t) == RECORD_TYPE || TREE_CODE (t) == UNION_TYPE)
    {
      /* Does this aggregate (or a nested one) contain placeholders?  */
      bool has_ph = false;
      for (tree f = TYPE_FIELDS (t); f; f = DECL_CHAIN (f))
	{
	  if (TREE_CODE (f) == FIELD_DECL)
	    {
	      tree ft = TREE_TYPE (f);
	      if (TREE_CODE (ft) == CZET_TPL_TYPE
		  || ((TREE_CODE (ft) == INTEGER_TYPE)
		      && TYPE_NAME (ft)
		      && TREE_CODE (TYPE_NAME (ft)) == IDENTIFIER_NODE
		      && bound->get (TYPE_NAME (ft)) != NULL)
		  || (TREE_CODE (ft) == RECORD_TYPE
		      || TREE_CODE (ft) == UNION_TYPE))
		{
		  has_ph = true;
		  break;
		}
      ...
      if (!has_ph)
	return t;
      /* Copy the aggregate and rebuild each member field.  */
      tree nt = copy_node (t);
      TYPE_SIZE (nt) = NULL_TREE;
      TYPE_SIZE_UNIT (nt) = NULL_TREE;
      TYPE_MAIN_VARIANT (nt) = nt;
      TYPE_NEXT_VARIANT (nt) = NULL_TREE;
      TYPE_CANONICAL (nt) = nt;
      tree newfields = NULL_TREE, tail = NULL_TREE;
      for (tree f = TYPE_FIELDS (t); f; f = DECL_CHAIN (f))
	{
	  if (TREE_CODE (f) != FIELD_DECL)
	    {
      ...
      finish_struct (input_location, nt, newfields, NULL_TREE, NULL, NULL);
      return nt;
    }
  walk_tree (&t, czet_subst_walk, bound, NULL);
  return t;
}

/* Build a concrete record type for generic TAG bound to TYPE_LIST
   (a TREE_LIST chain of concrete types), returning a laid-out RECORD_TYPE.
   Caches per <tag,types>, so repeated uses share one type.  */
tree
czet_instantiate_type (struct c_parser *parser ATTRIBUTE_UNUSED, tree tag,
			 tree type_list)
{
  tree tmpl = czet_get_type_template_type (tag);
  tree tparams = czet_get_type_template_params (tag);
  if (tmpl == NULL_TREE || tmpl == error_mark_node)
    return NULL_TREE;

  /* Bound map: parameter name -> concrete type.  */
  hash_map<tree, tree> *bound = new hash_map<tree, tree>;
  for (tree p = tparams, ty = type_list; p && ty;
       p = TREE_CHAIN (p), ty = TREE_CHAIN (ty))
    {
      if (p != error_mark_node && TREE_VALUE (p))
	bound->put (TREE_VALUE (p), TREE_VALUE (ty));
    }

  /* Dedup: build a *value-based* signature of the concrete types (code + name
     + size), so equivalent types from different node sources collapse to the
     same instance.  */
  char sig[1024];
  sig[0] = '\0';
  for (tree ty = type_list; ty; ty = TREE_CHAIN (ty))
    {
      tree ctysig = TREE_VALUE (ty);
      char tmp[128];
      const char *nm = "";
      if (ctysig && TYPE_NAME (ctysig)
	  && TREE_CODE (TYPE_NAME (ctysig)) == IDENTIFIER_NODE)
	nm = IDENTIFIER_POINTER (TYPE_NAME (ctysig));
      tree s = ctysig ? TYPE_SIZE (ctysig) : NULL_TREE;
      unsigned long sz = s && TREE_CODE (s) == INTEGER_CST
	? (unsigned long) TREE_INT_CST_LOW (s) : 0;
      snprintf (tmp, sizeof tmp, "%d|%s|%lu;", ctysig
		? (int) TREE_CODE (ctysig) : 0, nm, sz);
      if (strlen (sig) + strlen (tmp) >= sizeof sig)
	break;
      strcat (sig, tmp);
    }
  tree cache_head = NULL_TREE;
  if (czet_type_inst_cache)
    if (tree *slot = czet_type_inst_cache->get (tag))
      cache_head = *slot;
  for (tree c = cache_head; c; c = TREE_CHAIN (c))
    {
      tree ct = TREE_VALUE (c);
      if (ct && TREE_PURPOSE (c) && TREE_STRING_POINTER (TREE_PURPOSE (c))
	  && strcmp (TREE_STRING_POINTER (TREE_PURPOSE (c)), sig) == 0)
	{
	  delete bound;
	  return ct;
	}
  ...
  /* Fresh copy of the template record.  */
  tree nt = copy_node (tmpl);
  TYPE_SIZE (nt) = NULL_TREE;
  TYPE_SIZE_UNIT (nt) = NULL_TREE;
  TYPE_MAIN_VARIANT (nt) = nt;
  TYPE_NEXT_VARIANT (nt) = NULL_TREE;
  ...
  /* Copy fields in forward order, substituting each type.  */
  tree newfields = NULL_TREE, tail = NULL_TREE;
  for (tree f = TYPE_FIELDS (tmpl); f; f = DECL_CHAIN (f))
    {
      if (TREE_CODE (f) != FIELD_DECL)
	continue;
      tree nf = czet_copy_field (f, bound);
      DECL_CONTEXT (nf) = nt;
      DECL_CHAIN (nf) = NULL_TREE;
      if (tail)
	TREE_CHAIN (tail) = nf;
  ...
  /* Give the instance a unique tag name for dedup / diagnostics.  */
  static unsigned long counter;
  char tmp[64];
  snprintf (tmp, sizeof tmp, "%s__czet%lu", IDENTIFIER_POINTER (tag),
	    counter++);
  tree iname = get_identifier (tmp);
  TYPE_NAME (nt) = iname;

  /* Lay out the concrete record.  */
  finish_struct (input_location, nt, newfields, NULL_TREE, NULL, NULL);

  /* Cache under TAG with the signature string.  */
  if (czet_type_inst_cache == NULL)
    czet_type_inst_cache = new hash_map<tree, tree>;
  tree sig_node = build_string (strlen (sig) + 1, sig);
  tree entry = tree_cons (sig_node, nt, NULL_TREE);
  tree *slot = czet_type_inst_cache->get (tag);
  if (slot)
    TREE_CHAIN (entry) = *slot;
  czet_type_inst_cache->put (tag, entry);
...
/* -------------------------------------------------------------------------
   Operator overloading (_Operator).  Each definition is compiled as a plain
   function named __operator__<Type>__<suffix> and registered here so the
   expression rewriter (c-typeck.cc) can map `a + b` to a call of it.

   The registry key is the mangle suffix ("plus", "minus", "uminus", "pp",
   "assign", ...).  This keeps binary "-" (minus) and unary "-" (uminus)
   distinct even though they share the same user-visible operator string.
   ------------------------------------------------------------------------- */

/* Table of supported operators.  "mangle" is the name suffix used in the
   mangled identifier; "unary" says the operator takes a single operand;
   "mutate" says it modifies its operand in place (++/--); "bin" is the
   binary operator string that a compound assignment reuses ("+=" -> "+"),
   or NULL.  */
static const struct czet_operator_def
{
  const char *op;
  const char *mangle;
  bool unary;
  ...
  {
    { "+",  "plus",        false, false, NULL },
    { "-",  "minus",       false, false, NULL },
    { "*",  "mul",         false, false, NULL },
    { "/",  "div",         false, false, NULL },
    { "%",  "mod",         false, false, NULL },
    { "==", "eq",          false, false, NULL },
    { "!=", "ne",          false, false, NULL },
    { "<",  "lt",          false, false, NULL },
    { ">",  "gt",          false, false, NULL },
    { "<=", "le",          false, false, NULL },
    { ">=", "ge",          false, false, NULL },
    { "&",  "bitand",      false, false, NULL },
    { "|",  "bitor",       false, false, NULL },
    { "^",  "xor",         false, false, NULL },
    { "<<", "lshift",      false, false, NULL },
    { ">>", "rshift",      false, false, NULL },
    { "+=", "plus_assign", false, false, "+" },
    { "-=", "minus_assign",false, false, "-" },
    { "*=", "mul_assign",  false, false, "*" },
    { "/=", "div_assign",  false, false, "/" },
    { "%=", "mod_assign",  false, false, "%" },
    { "&=", "bitand_assign",false,false, "&" },
    { "|=", "bitor_assign",false, false, "|" },
    { "^=", "xor_assign",  false, false, "^" },
    { "<<=","lshift_assign",false,false, "<<" },
    { ">>=","rshift_assign",false,false, ">>" },
    { "-",  "uminus",      true,  false, NULL },
    { "!",  "lnot",        true,  false, NULL },
    { "~",  "bitnot",      true,  false, NULL },
    { "++", "pp",          true,  true,  NULL },
    { "--", "mm",          true,  true,  NULL },
    { "=",  "assign",      false, true,  NULL },
    { NULL, NULL,          false, false, NULL }
  };

/* Type -> (mangle-suffix IDENTIFIER -> FUNCTION_DECL).  */
static hash_map<tree, hash_map<tree, tree> *> *czet_operator_map;

/* True if OP names an operator that is intrinsically unary (++, --, !, ~,
   unary -).  Compound assignments are never unary.  */
bool
czet_operator_is_unary (const char *op)
{
  for (const struct czet_operator_def *d = czet_operator_defs;
       d->op; d++)
    if (strcmp (d->op, op) == 0)
      return d->unary;
  return false;
...
/* True if OP is a mutating operator (++/--/assign-style).  */
bool
czet_operator_is_unary_mutating (const char *op)
{
  for (const struct czet_operator_def *d = czet_operator_defs;
       d->op; d++)
    if (strcmp (d->op, op) == 0)
...
/* True if OP is the plain assignment operator "=".  */
bool
czet_operator_is_assign (const char *op)
{
  return strcmp (op, "=") == 0;
}

/* For a compound assignment operator ("+=", "-=", ...), return the binary
   operator string it reuses ("+", "-", ...); NULL for non-compound ops.  */
const char *
czet_operator_bin_for_assign (const char *op)
{
  for (const struct czet_operator_def *d = czet_operator_defs;
       d->op; d++)
    if (strcmp (d->op, op) == 0 && d->bin)
      return d->bin;
  return NULL;
}

/* Return the mangle suffix for OP.  For the ambiguous binary/unary "-",
   IS_UNARY selects "uminus" vs "minus".  Unknown operators map themselves
   verbatim (they are still rejectable upstream, but this allows future
   operators without a lexer change).  */
const char *
czet_operator_mangle_suffix (const char *op, bool is_unary)
{
  for (const struct czet_operator_def *d = czet_operator_defs;
       d->op; d++)
    if (strcmp (d->op, op) == 0)
      {
	if (is_unary && strcmp (d->op, "-") == 0)
	  return "uminus";
	return d->mangle;
      }
  return op;
...
/* Build the mangled FUNCTION_DECL name for TYPE and OP_SUFFIX:
   "__operator__<TypeName>__<suffix>".  */
tree
czet_operator_mangled_name (tree type, const char *op_suffix)
{
  const char *tn = "t";
  tree type_name = TYPE_NAME (type);
  if (type_name && TREE_CODE (type_name) == TYPE_DECL)
    type_name = DECL_NAME (type_name);
  else if (type_name && TREE_CODE (type_name) == IDENTIFIER_NODE)
    ;
  else
    type_name = NULL_TREE;
  if (type_name && TREE_CODE (type_name) == IDENTIFIER_NODE)
    tn = IDENTIFIER_POINTER (type_name);

  char buf[256];
  snprintf (buf, sizeof buf, "__operator__%s__%s", tn, op_suffix);
  return get_identifier (buf);
}

/* Register TYPE's operator overload FNDECL under mangle suffix OP_SUFFIX.  */
void
czet_register_operator (tree type, const char *op_suffix, tree fndecl)
{
  if (czet_operator_map == NULL)
    czet_operator_map = new hash_map<tree, hash_map<tree, tree> *>;
  hash_map<tree, tree> **slot = czet_operator_map->get (type);
  if (slot == NULL)
    {
      hash_map<tree, tree> *ops = new hash_map<tree, tree>;
      czet_operator_map->put (type, ops);
      ops->put (get_identifier (op_suffix), fndecl);
    }
  else
    (*slot)->put (get_identifier (op_suffix), fndecl);
}

/* Look up TYPE's operator overload with mangle suffix OP_SUFFIX, or NULL.  */
tree
czet_lookup_operator (tree type, const char *op_suffix)
{
  if (czet_operator_map == NULL)
    return NULL_TREE;
  hash_map<tree, tree> **slot = czet_operator_map->get (type);
  if (slot == NULL)
    return NULL_TREE;
  tree *decl = (*slot)->get (get_identifier (op_suffix));
  return decl ? *decl : NULL_TREE;
}
...
/* CZet primitive reflection types:
   ...
   Always registered so `reflect_t' works without any header.  */
static void
czet_init_reflect_types (void)
{
  location_t loc = BUILTINS_LOCATION;
  class c_struct_parse_info *info_r = NULL, *info_f = NULL;
  tree rid = get_identifier ("reflect_t");
  tree fid = get_identifier ("reflect_field_t");

  tree rtype = start_struct (loc, RECORD_TYPE, rid, &info_r);
  tree ftype = start_struct (loc, RECORD_TYPE, fid, &info_f);
  if (!rtype || !ftype)
    return;

  tree rptr = build_pointer_type (rtype);
  tree fptr = build_pointer_type (ftype);

  tree ff_name = build_decl (loc, FIELD_DECL, get_identifier ("name"),
			     const_string_type_node);
  tree ff_offset = build_decl (loc, FIELD_DECL, get_identifier ("offset"),
			       long_unsigned_type_node);
  tree ff_type = build_decl (loc, FIELD_DECL, get_identifier ("type"), rptr);
  DECL_FIELD_CONTEXT (ff_name) = ftype;
  DECL_CHAIN (ff_name) = ff_offset;
  DECL_FIELD_CONTEXT (ff_offset) = ftype;
  DECL_CHAIN (ff_offset) = ff_type;
  DECL_FIELD_CONTEXT (ff_type) = ftype;
  finish_struct (loc, ftype, ff_name, NULL_TREE, info_f);
  tree fdecl = build_decl (loc, TYPE_DECL, fid, ftype);
  set_underlying_type (fdecl);
  pushdecl (fdecl);

  tree rf_kind = build_decl (loc, FIELD_DECL, get_identifier ("kind"),
			     integer_type_node);
  tree rf_size = build_decl (loc, FIELD_DECL, get_identifier ("size"),
			     long_unsigned_type_node);
  tree rf_align = build_decl (loc, FIELD_DECL, get_identifier ("align"),
			      long_unsigned_type_node);
  tree rf_name = build_decl (loc, FIELD_DECL, get_identifier ("name"),
			     const_string_type_node);
  tree rf_count = build_decl (loc, FIELD_DECL, get_identifier ("count"),
			      long_unsigned_type_node);
  tree rf_elem = build_decl (loc, FIELD_DECL, get_identifier ("elem"), rptr);
  tree rf_fields = build_decl (loc, FIELD_DECL, get_identifier ("fields"),
			       fptr);
  DECL_FIELD_CONTEXT (rf_kind) = rtype;
  DECL_CHAIN (rf_kind) = rf_size;
  DECL_FIELD_CONTEXT (rf_size) = rtype;
  DECL_CHAIN (rf_size) = rf_align;
  DECL_FIELD_CONTEXT (rf_align) = rtype;
  DECL_CHAIN (rf_align) = rf_name;
  DECL_FIELD_CONTEXT (rf_name) = rtype;
  DECL_CHAIN (rf_name) = rf_count;
  DECL_FIELD_CONTEXT (rf_count) = rtype;
  DECL_CHAIN (rf_count) = rf_elem;
  DECL_FIELD_CONTEXT (rf_elem) = rtype;
  DECL_CHAIN (rf_elem) = rf_fields;
  DECL_FIELD_CONTEXT (rf_fields) = rtype;
  finish_struct (loc, rtype, rf_kind, NULL_TREE, info_r);
  tree rdecl = build_decl (loc, TYPE_DECL, rid, rtype);
  set_underlying_type (rdecl);
  pushdecl (rdecl);
}
...
c_init_decl_processing (void)
{
  ...
  /* CZet primitive reflection types (always available).  */
  czet_init_reflect_types ();
		...
		enum deprecated_states deprecated_state)
{
  ...
  /* CZet: a type slot that is still an unresolved IDENTIFIER_NODE was a
     deferred generic (template) type-parameter name.  Bound it now that the
     declarator's <T,U,W> list is known.  */
  if (flag_isoczet && type && TREE_CODE (type) == IDENTIFIER_NODE)
    {
      tree ph = czet_param_placeholder (type);
      if (ph)
	type = declspecs->type = ph;
      else
	{
	  tree t = lookup_name (type);
	  if (t && TREE_CODE (t) == TYPE_DECL && TREE_TYPE (t) != error_mark_node)
	    {
	      declspecs->type = TREE_TYPE (t);
	      declspecs->typedef_p = true;
	      type = declspecs->type;
	    }
	  else
	    {
	      error_at (input_location, "unknown type name %qE", type);
	      type = declspecs->type = error_mark_node;
	    }
	}
    }
  ...
  *expr_const_operands = declspecs->expr_const_operands;
  ...
  if (funcdef_flag
	...
	error_at (loc, "function definition declared %qs",
	 ...
	 reaches here, except under the CZet extension dialect.  */
      if (constexprp)
	gcc_assert (flag_isoczet);
      if (storage_class == csc_auto
	  || storage_class == csc_register
	  || storage_class == csc_typedef)
	storage_class = csc_none;
...
grokparms (struct c_arg_info *arg_info, bool funcdef_flag)
{
  ...
  if (arg_types == error_mark_node)
	   ...
	   parm = DECL_CHAIN (parm), typelt = TREE_CHAIN (typelt), parmno++)
	{
	  ...
	  if (TREE_CODE (type) != CZET_TPL_TYPE
	      && !COMPLETE_TYPE_P (type))
	    {
	      if (funcdef_flag)
		{
		...
		tree attributes)
{
  ...
  /* CZet: a generic definition was parsed with an active type-parameter
     scope.  Attach the parameter list to the decl so it can be monomorphized
     on demand, and mark the decl so it is not emitted as-is.  During an
     instantiation re-parse this is skipped: the concrete instance is a real,
     emit-able function.  */
  if (czet_current_generic_parms && flag_isoczet
      && czet_binding_template == NULL_TREE)
    {
      czet_set_generic_params (decl1, czet_current_generic_parms);
      C_DECL_GENERIC_TEMPLATE (decl1) = 1;
      czet_push_generic_fn (decl1);
    }
  else if (czet_binding_template != NULL_TREE
	   && TREE_TYPE (decl1) != error_mark_node)
    {
      /* This is the re-parse of a concrete instantiation.  Give the inner
	 function the same mangled name as the instance decl referenced at
	 the call site, so the symbol the callee resolves to is defined.  */
      if (czet_replay_decl && DECL_NAME (czet_replay_decl))
	DECL_NAME (decl1) = DECL_NAME (czet_replay_decl);
      czet_last_instance = decl1;
    }
  ...
  /* CZet extension: record a constexpr function definition so it may
     be evaluated at compile time.  */
  if (declspecs->constexpr_p && flag_isoczet)
    C_DECL_DECLARED_CONSTEXPR_FN (decl1) = 1;
...
store_parm_decls_newstyle (tree fndecl, const struct c_arg_info *arg_info)
{
  ...
  if (czet_replay_decl != NULL_TREE
      && TREE_TYPE (czet_replay_decl) != NULL_TREE
      && TREE_TYPE (czet_replay_decl) != error_mark_node
      && TREE_CODE (TREE_TYPE (czet_replay_decl)) == FUNCTION_TYPE)
    {
      /* The replayed body would otherwise inherit the shared template's
	 parameter types (the generic decl of the same name wins the
	 declarator type-match and its arg nodes carry the raw template
	 references).  Re-stamp each parameter with the concrete instance
	 types so `p->b` etc. resolve records per instantiation.  */
      tree fty = TREE_TYPE (czet_replay_decl);
      tree at = TYPE_ARG_TYPES (fty);
      for (tree d = arg_info->parms; d; d = DECL_CHAIN (d))
	{
	  if (at == NULL_TREE || at == void_list_node)
	    break;
	  TREE_TYPE (d) = TREE_VALUE (at);
	  at = TREE_CHAIN (at);
	}
      TREE_TYPE (fndecl) = fty;
    }
...
store_parm_decls_oldstyle (tree fndecl, const struct c_arg_info *arg_info)
{
  ...
  for (b = current_scope->bindings; b; b = b->prev)
	...
	continue;
      ...
      if (TREE_CODE (TREE_TYPE (parm)) != CZET_TPL_TYPE
	  && TREE_TYPE (parm) != error_mark_node
...
finish_function (location_t end_loc)
{
  ...
  if (flag_isoczet && czet_current_generic_parms)
    czet_pop_generic_fn ();
  ...
  if (DECL_INITIAL (fndecl) && DECL_INITIAL (fndecl) != error_mark_node
      ...
      && !(flag_isoczet && C_DECL_GENERIC_TEMPLATE (fndecl)))
    {
		    ...
		    struct c_typespec spec)
{
  ...
  if (specs->type || specs->typespec_word != cts_none
  ...
  else if (TREE_CODE (type) == IDENTIFIER_NODE)
    {
      if (flag_isoczet)
	{
	  /* CZet: the name may be a generic (template) type parameter that
	     is only bound once the declarator's <T,U,W> list has been seen.
	     Defer resolution to grokdeclarator.  */
	  specs->type = type;
	  specs->locations[cdw_typespec] = loc;
	}
      else

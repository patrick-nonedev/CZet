...
/* Set on FUNCTION_DECLs declared as 'constexpr' (CZet extension:
   such functions may be evaluated at compile time).  */
#define C_DECL_DECLARED_CONSTEXPR_FN(DECL) \
  DECL_LANG_FLAG_8 (FUNCTION_DECL_CHECK (DECL))

/* Set on a FUNCTION_DECL that is a generic (template) function in the
   CZet dialect: its prototype contains CZET_TPL_TYPE placeholders and
   it is never emitted directly, only instantiated per unique type-argument
   list (monomorphization).  SAFE: bit 7 of a FUNCTION_DECL is otherwise
   only used by C_DECL_UNDERSPECIFIED which applies to objects, never
   functions.  */
#define C_DECL_GENERIC_TEMPLATE(DECL) \
  DECL_LANG_FLAG_7 (FUNCTION_DECL_CHECK (DECL))

/* Set on a FUNCTION_DECL that is an operator overload defined with the
   _Operator construct in the CZet dialect.  SAFE: bit 0 of a FUNCTION_DECL
   is unused by the C frontend.  */
#define C_DECL_OPERATOR_OVERLOAD(DECL) \
  DECL_LANG_FLAG_0 (FUNCTION_DECL_CHECK (DECL))

/* Type-parameter list (a list of IDENTIFIER_NODEs) of a generic function, in
   order.  Stored on its FUNCTION_DECL via DECL_ARGUMENTS of the template
   decl? NO - see c-decl.cc.  Attached through the attribute mechanism.  */
extern tree czet_generic_params (tree fn);
extern void czet_set_generic_params (tree fn, tree params);
extern bool czet_is_generic (tree fn);
extern tree czet_current_generic_params_scope (void);
extern void czet_push_generic_fn (tree fn);
extern void czet_pop_generic_fn (void);
extern void czet_set_generic_scope (tree params);
extern void czet_set_instantiation_bindings (tree templ, tree types);
extern void czet_clear_instantiation_bindings (void);
extern tree czet_instance_name (tree templ);
extern void czet_set_binding_template (tree templ);
extern bool czet_instantiation_active (void);
extern tree czet_last_instance;
extern tree czet_replay_decl;
extern tree czet_materialize (tree templ, tree type_list);
extern tree czet_build_instance_decl (tree templ, tree type_list);
struct c_parser;
extern tree czet_instantiate (struct c_parser *parser, tree templ, tree type_list);
extern void czet_flush_instances (struct c_parser *parser);
extern tree czet_make_type_parm (tree name);
extern tree czet_param_placeholder (tree name);
/* Type (struct/union) templates.  */
extern void czet_set_type_template (tree tag, tree params, tree template_type);
extern tree czet_get_type_template_type (tree tag);
extern tree czet_get_type_template_params (tree tag);
extern tree czet_instantiate_type (struct c_parser *parser, tree tag, tree type_list);
/* True iff TYPE is itself a CZET_TPL_TYPE placeholder.  */
#define CZET_TPL_TYPE_P(TYPE) (TREE_CODE (TYPE) == CZET_TPL_TYPE)

/* Operator overloading (_Operator) support.  The "mangle suffix" is the
   per-operation part of the mangled name (e.g. "plus", "uminus", "pp",
   "assign"); it disambiguates binary "-" (minus) from unary "-" (uminus).  */
extern const char *czet_operator_mangle_suffix (const char *op, bool is_unary);
extern bool czet_operator_is_unary (const char *op);
extern bool czet_operator_is_unary_mutating (const char *op);
extern bool czet_operator_is_assign (const char *op);
extern const char *czet_operator_bin_for_assign (const char *op);
extern tree czet_operator_mangled_name (tree type, const char *op_suffix);
extern void czet_register_operator (tree type, const char *op_suffix, tree fndecl);
extern tree czet_lookup_operator (tree type, const char *op_suffix);

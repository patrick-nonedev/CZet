...
/* -------------------------------------------------------------------------
   CZet: user-defined operator overloading (_Operator).

   Each `_Operator` definition was compiled to a plain function
   `__operator__<Type>__<suffix>` and registered by the parser.  The
   helpers below rewrite the builtin expression construction so that an
   operator application on a user-defined type becomes a call of the
   overload: `a + b` -> `__operator__Vec2__plus (&a, &b)`, `++a` ->
   `__operator__Vec2__pp (&a)`, `a += b` -> `*(__operator__Vec2__plus_assign
   (&a, &b))`, and `a = b` -> `__operator__Vec2__assign (&a, &b)`.

   Each helper returns the rewritten expression, or NULL_TREE when no
   user overload applies (the caller then keeps its builtin semantics).
   ------------------------------------------------------------------------- */

/* Mangle suffix for a binary expression code, or NULL if the code cannot
   be overloaded.  */
static const char *
czet_binary_op_suffix (enum tree_code code)
{
  switch (code)
    {
    case PLUS_EXPR:         return "plus";
    case MINUS_EXPR:        return "minus";
    case MULT_EXPR:         return "mul";
    case TRUNC_DIV_EXPR:
    case EXACT_DIV_EXPR:
    case RDIV_EXPR:         return "div";
    case TRUNC_MOD_EXPR:    return "mod";
    case EQ_EXPR:           return "eq";
    case NE_EXPR:           return "ne";
    case LT_EXPR:           return "lt";
    case GT_EXPR:           return "gt";
    case LE_EXPR:           return "le";
    case GE_EXPR:           return "ge";
    case BIT_AND_EXPR:      return "bitand";
    case BIT_IOR_EXPR:      return "bitor";
    case BIT_XOR_EXPR:      return "xor";
    case LSHIFT_EXPR:       return "lshift";
    case RSHIFT_EXPR:       return "rshift";
    default:                return NULL;
    }
}

/* Mangle suffix for the compound-assign version of a binary expression
   code ("plus" -> "plus_assign"), or NULL if not overloadable.  */
static const char *
czet_compound_op_suffix (enum tree_code code)
{
  const char *bin = czet_binary_op_suffix (code);
  if (bin == NULL)
    return NULL;
  static char buf[32];
  snprintf (buf, sizeof buf, "%s_assign", bin);
  return buf;
}

/* Mangle suffix for a unary expression code, or NULL if not overloadable.  */
static const char *
czet_unary_op_suffix (enum tree_code code)
{
  switch (code)
    {
    case NEGATE_EXPR:       return "uminus";
    case TRUTH_NOT_EXPR:    return "lnot";
    case BIT_NOT_EXPR:      return "bitnot";
    case PREINCREMENT_EXPR:
    case POSTINCREMENT_EXPR: return "pp";
    case PREDECREMENT_EXPR:
    case POSTDECREMENT_EXPR: return "mm";
    default:                return NULL;
    }
}

/* True if TYPE is a user-defined type that can own overloads.  */
static bool
czet_operable_type (tree type)
{
...
/* True if EXPR is addressable by the C rules (i.e. can be passed by
   address to an operator overload without making a copy).  */
static bool
czet_lvalue_p (tree expr)
{
...
/* Nonzero while the rewriter is building its own internal copies
   (materializing temporaries); the operator hooks are suppressed during
   that so an (already registered) "=" overload cannot re-enter the
   rewriter recursively.  */
static bool czet_building_op_copy;

/* Make OPERAND passable by address, materializing a temporary that is
   initialized with a copy of a non-addressable rvalue (a folded result
   of another overload, a compound literal, ...).  Returns an lvalue
   expression; for a materialized rvalue the expression is
   `(init, tmp)` so that the copy runs before the address is used.
   Returns NULL_TREE when the temporary cannot be created.  */
static tree
czet_addressable_operand (location_t location, tree operand)
{
  if (operand == error_mark_node)
    return NULL_TREE;
  if (czet_lvalue_p (operand))
    return operand;

  tree type = TREE_TYPE (operand);
  if (!COMPLETE_TYPE_P (type) || cfun == NULL)
    return NULL_TREE;

  tree tmp = create_tmp_var (type, "oparg");
  TREE_ADDRESSABLE (tmp) = 1;
  suppress_warning (tmp);
  bool save = czet_building_op_copy;
  czet_building_op_copy = true;
  tree init = build_modify_expr (location, tmp, NULL_TREE, NOP_EXPR,
				 location, operand, NULL_TREE);
  czet_building_op_copy = save;
  if (init == error_mark_node)
    return NULL_TREE;
  return build2 (COMPOUND_EXPR, type, init, tmp);
}

/* Take the address of an (addressable) expression M.  When M is a
   materialized rvalue `(init, tmp)`, the address is of the temporary,
   keeping the copy as a preceding side effect.  */
static tree
czet_addr_of (location_t location, tree m)
{
  if (TREE_CODE (m) == COMPOUND_EXPR)
    {
      tree init = TREE_OPERAND (m, 0);
      tree tmp = remove_c_maybe_const_expr (TREE_OPERAND (m, 1));
      tree addr = build_unary_op (location, ADDR_EXPR, tmp, 0);
      return build2 (COMPOUND_EXPR, TREE_TYPE (addr), init, addr);
    }
  return build_unary_op (location, ADDR_EXPR, m, 0);
}

/* Copy SOURCE into a fresh temporary, returning an expression that
   evaluates the copy and yields the temporary (value semantics).  */
static tree
czet_copy_to_temp (location_t location, tree source)
{
  tree type = TREE_TYPE (source);
  tree tmp = create_tmp_var (type, "opcopy");
  suppress_warning (tmp);
  bool save = czet_building_op_copy;
  czet_building_op_copy = true;
  tree init = build_modify_expr (location, tmp, NULL_TREE, NOP_EXPR,
				 location, source, NULL_TREE);
  czet_building_op_copy = save;
  if (init == error_mark_node)
    return NULL_TREE;
  return build2 (COMPOUND_EXPR, type, init, tmp);
...
/* Rewrite a binary operation, or return NULL_TREE if the builtin
   semantics should apply.  */
static tree
czet_rewrite_binary_op (location_t location, enum tree_code code,
			  tree op0, tree op1)
{
  const char *suffix = czet_binary_op_suffix (code);
  if (suffix == NULL)
    return NULL_TREE;

  tree t0 = TREE_TYPE (op0);
  tree t1 = TREE_TYPE (op1);
  if (!czet_operable_type (t0) || !czet_operable_type (t1))
    return NULL_TREE;
  if (!comptypes (TYPE_MAIN_VARIANT (t0), TYPE_MAIN_VARIANT (t1)))
    return NULL_TREE;

  tree fndecl = czet_lookup_operator (TYPE_MAIN_VARIANT (t0), suffix);
  if (fndecl == NULL_TREE)
    return NULL_TREE;

  op0 = require_complete_type (location, op0);
  op1 = require_complete_type (location, op1);
  tree m0 = czet_addressable_operand (location, op0);
  tree m1 = czet_addressable_operand (location, op1);
  if (m0 == NULL_TREE || m1 == NULL_TREE)
    return NULL_TREE;
  tree a0 = czet_addr_of (location, m0);
  tree a1 = czet_addr_of (location, m1);
  return build_call_expr_loc (location, fndecl, 2, a0, a1);
}

/* Rewrite a unary operation, or return NULL_TREE if the builtin
   semantics should apply.  */
static tree
czet_rewrite_unary_op (location_t location, enum tree_code code, tree arg)
{
  const char *suffix = czet_unary_op_suffix (code);
  if (suffix == NULL)
    return NULL_TREE;

  tree type = TREE_TYPE (arg);
  if (!czet_operable_type (type))
    return NULL_TREE;

  tree fndecl = czet_lookup_operator (TYPE_MAIN_VARIANT (type), suffix);
  if (fndecl == NULL_TREE)
    return NULL_TREE;

  arg = require_complete_type (location, arg);

  bool mutating = (code == PREINCREMENT_EXPR || code == POSTINCREMENT_EXPR
		   || code == PREDECREMENT_EXPR || code == POSTDECREMENT_EXPR);
  bool ret_void = VOID_TYPE_P (TREE_TYPE (TREE_TYPE (fndecl)));

  if (mutating && ret_void)
    {
      /* The overload mutates its operand in place and returns void.  The
	 expression keeps the C value semantics:

	   ++a:  ( op(&a);  a )  -- the (new) operand value
	   a++:  ( old = a; op(&a); old )  -- the old operand value  */
      tree m0 = czet_addressable_operand (location, arg);
      if (m0 == NULL_TREE)
	return NULL_TREE;
      bool is_postfix = (code == POSTINCREMENT_EXPR
			 || code == POSTDECREMENT_EXPR);
      tree call = build_call_expr_loc (location, fndecl, 1,
				       czet_addr_of (location, m0));
      if (!is_postfix)
	return build2 (COMPOUND_EXPR, type, call, m0);
      tree old = czet_copy_to_temp (location, m0);
      if (old == NULL_TREE)
	return NULL_TREE;
      tree seq = build2 (COMPOUND_EXPR, void_type_node, old, call);
      return build2 (COMPOUND_EXPR, type, seq, TREE_OPERAND (old, 1));
    }
  ...
  if (m0 == NULL_TREE)
    return NULL_TREE;
  tree a0 = czet_addr_of (location, m0);
  return build_call_expr_loc (location, fndecl, 1, a0);
}

/* Rewrite an assignment (`=` or a compound assignment), or return
   NULL_TREE if the builtin semantics should apply.  */
static tree
czet_rewrite_modify_expr (location_t location, tree lhs,
			    enum tree_code modifycode, tree rhs)
{
  const char *suffix = (modifycode == NOP_EXPR
			? "assign"
			: czet_compound_op_suffix (modifycode));
  if (suffix == NULL)
    return NULL_TREE;

  tree lhstype = TREE_TYPE (lhs);
  if (!czet_operable_type (lhstype))
    return NULL_TREE;

  tree rhstype = TREE_TYPE (rhs);
  if (!czet_operable_type (rhstype)
      || !comptypes (TYPE_MAIN_VARIANT (lhstype), TYPE_MAIN_VARIANT (rhstype)))
    return NULL_TREE;

  tree fndecl = czet_lookup_operator (TYPE_MAIN_VARIANT (lhstype), suffix);
  if (fndecl == NULL_TREE)
    return NULL_TREE;

  /* The LHS must be a real lvalue (assigning to an rvalue is diagnosed
     by the builtin path), and the RHS is passed by address: non-lvalue
     rvalues are copied into a temporary first.  */
  if (!czet_lvalue_p (lhs))
    return NULL_TREE;

  lhs = require_complete_type (location, lhs);
  rhs = require_complete_type (location, rhs);
  tree m1 = czet_addressable_operand (location, rhs);
  if (m1 == NULL_TREE)
    return NULL_TREE;
  tree a0 = build_unary_op (location, ADDR_EXPR, lhs, 0);
  tree a1 = czet_addr_of (location, m1);
  tree call = build_call_expr_loc (location, fndecl, 2, a0, a1);

  /* A compound assignment overload returns a pointer to its first operand
     (modified in place); dereference it so the assignment expression keeps
     yielding the (new) LHS as an lvalue like in C.  */
  if (modifycode != NOP_EXPR && POINTER_TYPE_P (TREE_TYPE (fndecl)))
    return build_indirect_ref (location, call, RO_UNARY_STAR);
  return call;
}
		...
		bool noconvert)
{
  ...
  /* CZet: user-defined unary operators take precedence over the builtin
     semantics for `OP arg` when ARG is of a registered user-defined type.  */
  if (flag_isoczet && !czet_building_op_copy)
    {
      tree over = czet_rewrite_unary_op (location, code, arg);
      if (over != NULL_TREE)
	return over;
    }
		   ...
		   location_t rhs_loc, tree rhs, tree rhs_origtype)
{
  ...
  /* CZet: user-defined assignment operators (`=` and the compound
     assignments) take precedence over the builtin semantics for
     user-defined LHS/RHS types.  */
  if (flag_isoczet && !czet_building_op_copy)
    {
      tree over = czet_rewrite_modify_expr (location, lhs, modifycode, rhs);
      if (over != NULL_TREE)
	return over;
    }
  ...
  if (INTEGRAL_TYPE_P (type))
    {
	 ...
	 representable in the target type.  */
      /* CZet extension: a constexpr-function call may have been folded
	 to a constant after int_const_expr was computed, so recompute it
	 from the folded initializer.  */
      if (!int_const_expr && TREE_CODE (init) == INTEGER_CST)
	int_const_expr = true;
      if (!int_const_expr)
	{
		 ...
		 tree orig_op0, tree orig_op1, bool convert_p)
{
  ...
  /* CZet: user-defined binary operators take precedence over the
     builtin semantics for `op0 OP op1` when both operands are of the
     same registered user-defined type.  */
  if (flag_isoczet && !czet_building_op_copy)
    {
      tree over = czet_rewrite_binary_op (location, code, op0, op1);
      if (over != NULL_TREE)
	return over;
    }
